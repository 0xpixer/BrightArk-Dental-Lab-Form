...
export function EditOrderForm({ orderId, embedded = false }: { orderId: string; embedded?: boolean }) {
  const [data, setData] = useState<{ values: OrderFormValues; files: Record<string, string> } | null>(null)
  const [error, setError] = useState<string | null>(null)
  const load = useCallback(async () => { const response = await fetch(`/api/portal/orders/${orderId}`); const payload = await response.json(); if (!response.ok) { setError(payload.error ?? 'Unable to load order'); return }; const order = payload.order; const deliveryAddress = order.address ?? ''; const billAddress = order.billingAddress ?? deliveryAddress; const cloudDriveLinks = order.cloudDriveLinks ?? (order.cloudDriveLink ? [order.cloudDriveLink] : ['']); setData({ values: { ...defaultFormValues, dentist: order.dentist, clinic: order.clinic, email: order.email, address: deliveryAddress, billAddress, billAddressSameAsDelivery: billAddress === deliveryAddress, patient: order.patientName, patientAge: order.patientAge ?? '', sex: order.sex?.toLowerCase() === 'female' ? 'female' : order.sex?.toLowerCase() === 'male' ? 'male' : '', treatmentCategory: categoryByLabel[order.treatmentType] ?? 'fixed', ...(order.treatmentData ?? {}), ...(order.toothSelection ?? {}), instructions: order.instructions ?? '', cloudDriveLink: order.cloudDriveLink ?? '', cloudDriveLinks }, files: order.fileUrls ?? {} }) }, [orderId])
  useEffect(() => { load() }, [load])
  if (!data) return <p className="p-6 text-text-muted">{error ?? 'Loading order...'}</p>
  return <OrderForm orderId={orderId} initialValues={data.values} initialFileUrls={data.files} embedded={embedded} />
}
