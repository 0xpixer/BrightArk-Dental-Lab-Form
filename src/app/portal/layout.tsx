...
export default async function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="flex min-h-screen bg-bg"><PortalSidebar username={session.user.username} role={session.user.role} /><div className="flex min-w-0 flex-1 flex-col"><header className="sticky top-0 z-10 border-b border-border bg-surface px-4 py-4 shadow-sm md:px-6"><p className="text-sm font-medium text-text-muted">BrightArk Doctor Portal</p></header><main className="flex-1 overflow-auto p-3 sm:p-4 md:p-6">{children}</main></div></div>
}
