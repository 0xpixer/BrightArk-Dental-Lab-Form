...
export async function GET(_request: Request, { params }: { params: { id: string } }) {
  ...
  if (!isPortalRole(role) && !isSalesRole(role)) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  const id = parseInt(params.id, 10)
  if (isNaN(id)) return NextResponse.json({ error: 'Invalid order ID' }, { status: 400 })
  const userId = parseInt(session!.user.id, 10)
  const order = isSalesRole(role)
...
export async function PATCH(request: Request, { params }: { params: { id: string } }) {
  ...
  const ownerId = isSalesRole(role) ? order.submittedBy : await getOrderOwnerId(userId, role)
  const db = getDb()
